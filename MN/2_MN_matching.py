...
for i, fda_row in fda_df.iterrows():
    ...
    if temp_sos.empty == False:
        ...
        if out.name_score.max() > .99:
            ...
            fda_df.loc[i,'SoS_record'] = match['File Number']
            fda_df.loc[i,'URL'] = match['URL']
            fda_df.loc[i,'entity'] = match['Entity Name']
            fda_df.loc[i,'agent_name'] = match['Registered Agent(s)']
            fda_df.loc[i,'store_address'] = match['Principal Place of Business Address']
            address = try_tag(match['Registered Office Address'])
            if address != '':
...
fda_df.to_csv('C:/Users/lpatterson/AnacondaProjects/Tribal_Master/step_4_work/' +
    'MN/MN_FDA_matches.csv',index=False)
